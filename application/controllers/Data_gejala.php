...
class Data_gejala extends CI_Controller
{
	...
	public function index()
	{
		if (isset($_GET['tambah'])) {
			...
			$this->load->view('module/module_sp/view_tambah_gejala', [
				'rows2' => $rows2, 'model' => $this->model
			]);
		} else if (isset($_GET['ubah'])) {
			$rows2 = $this->model->select2();
			$this->load->view('module/module_sp/view_ubah_gejala', [
				'rows2' => $rows2, 'model' => $this->model
			]);
		} else {
			$this->select();
	...
	public function create()
	{
		if (isset($_POST['btnSubmit'])) {
			...
			$this->model->jawaban_ya = $_POST['jawaban_ya'];
			$this->model->jawaban_tidak = $_POST['jawaban_tidak'];
			$this->model->insert();
